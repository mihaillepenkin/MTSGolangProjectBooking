...
var (
	...
	testBookingInfo    = &object.BookingInfo{User: *testUser, HotelName: testHotelName, RoomNumber: testRoomNumber, CheckIn: testDurations[0][0], CheckOut: testDurations[0][1]}
...
func (m *MockBookRepo) IsIntersected(ctx context.Context, hotelName string, hotelRoom string, checkIn time.Time, checkOut time.Time) (bool, error) {
...
func (m *MockBookRepo) GetByBookingInfo(ctx context.Context, bookingInfo *object.BookingInfo) (*booking.Booking, error) {
	if bookingInfo.HotelName == testHotelName && bookingInfo.RoomNumber == testRoomNumber {
		return testBooking, nil
	}
	return nil, fmt.Errorf("booking info not found")
}

func (m *MockBookRepo) GetByHotel(ctx context.Context, hotelName string) ([]*booking.Booking, error) {
	//TODO implement me
	panic("implement me")
}
...
func (m *MockBookRepo) GetDurationsByRoom(ctx context.Context, hotelName string, roomNumber string) ([][]time.Time, error) {
	//TODO implement me
	panic("implement me")
}
...
func TestBookingSaver_BookRoom(t *testing.T) {
	...
	_, err := testSaver.BookRoom(ctx, &object.BookingInfo{HotelName: "2"})
	assert.Assert(t, errors.Is(err, error3.ErrBookingIsIntersected), "book room error")
	_, err = testSaver.BookRoom(ctx, &object.BookingInfo{HotelName: testHotelName, RoomNumber: "2"})
